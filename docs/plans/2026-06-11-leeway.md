...
**Files:**
- Create: `requirements.txt`, `requirements-dev.txt`, `src/leeway/__init__.py`, `tests/__init__.py`, `pytest.ini`, `.gitignore`
...
**Files:**
- Create: `src/leeway/config.py`, `tests/test_config.py`

**Step 1 — testes que falham:**
```python
import pytest
from leeway.config import Settings, validate
...
**Step 1 — teste:**
```python
from leeway.engine import commands_to_activate, command_to_deactivate
...
**Files:** Create `src/leeway/batt.py`
...
**Files:** Create `src/leeway/app.py`
...
- `bin/leeway`: cria/ativa `.venv`, garante deps, roda `python -m leeway.app`.
...
- `python -c "import leeway.app"` (com rumps instalado) → importa sem erro.
