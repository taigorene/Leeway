...
CONFIG_PATH = Path(
    os.path.expanduser("~/.config/sailing-mode/config.json")
)


@dataclass
class Settings:
    """Limites de carga: ``lower`` (início) e ``upper`` (fim), em %."""
...
def load(path: Path = CONFIG_PATH) -> Settings:
    ...
    try:
        ...
        return Settings(lower=lower, upper=upper)
...
def save(settings: Settings, path: Path = CONFIG_PATH) -> None:
    ...
    path.write_text(json.dumps({"lower": settings.lower, "upper": settings.upper}))
